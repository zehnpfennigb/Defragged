...
UCLASS(config=Game)
class DEFRAGGED_API ADFGameSession : public AGameSession
{
...
protected:
	...
	/** Host Settings */
	TSharedPtr<class FDFOnlineGameSettings> HostSettings;
	/** Search Settings */
	TSharedPtr<class FDFOnlineSearchSettings> SearchSettings;
	...
	DECLARE_EVENT_TwoParams(ADFGameSession, FOnCreatePresenceSessionComplete, FName /*SessionName*/, bool /*bWasSuccessful*/);
	FOnCreatePresenceSessionComplete CreatePresenceSessionCompleteEvent;

	/** The even that is triggered when joining a session */
	DECLARE_EVENT_OneParam(ADFGameSession, FOnJoinSessionComplete, bool /*bWasSuccessful*/);
