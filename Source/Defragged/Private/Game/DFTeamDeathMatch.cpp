...
#include "Defragged.h"
#include "Game/DFTeamDeathMatch.h"

ADFTeamDeathMatch::ADFTeamDeathMatch(const class FPostConstructInitializeProperties& PCIP) : Super(PCIP)
{
...
void ADFTeamDeathMatch::BeginPlay()
{
	Super::BeginPlay();
}
...
void ADFTeamDeathMatch::InitGameState()
{
	Super::InitGameState();
...
/** SPAWNING */
AActor* ADFTeamDeathMatch::ChoosePlayerStart(AController* Player)
{
...
bool ADFTeamDeathMatch::IsSpawnPointAllowed(APlayerStart* SpawnPoint, AController* Player) const
{
	if (Player)
	{
...
bool ADFTeamDeathMatch::ShouldSpawnAtStartSpot(AController* Player)
{
	return false;	
}

UClass* ADFTeamDeathMatch::GetDefaultPawnClassForController(AController* InController)
{
...
/** DAMAGE AND TEAMS */
bool ADFTeamDeathMatch::CanDealDamage(class ADFPlayerState* Shooter, class ADFPlayerState* DamagedPlayer) const
{
	return Shooter && DamagedPlayer && (DamagedPlayer == Shooter || DamagedPlayer->GetTeamNum() != Shooter->GetTeamNum());
}

int32 ADFTeamDeathMatch::ChooseTeam(ADFPlayerState* ForPlayerState) const
{
...
void ADFTeamDeathMatch::DetermineWinner()
{
...
bool ADFTeamDeathMatch::IsWinner(class ADFPlayerState* PlayerState) const
{
	return PlayerState && PlayerState->GetTeamNum() == WinningTeam;
}

void ADFTeamDeathMatch::Killed(AController* Killer, AController* KilledPlayer, APawn* KilledPawn)
{
