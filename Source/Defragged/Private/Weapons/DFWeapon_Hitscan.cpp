...
#include "Defragged.h"
#include "Weapons/DFWeapon_Hitscan.h"
#include "DrawDebugHelpers.h"


ADFWeapon_Hitscan::ADFWeapon_Hitscan(const class FPostConstructInitializeProperties& PCIP)
	: Super(PCIP)
{
...
/** Server Hits */
bool ADFWeapon_Hitscan::ServerNotifyHit_Validate(const FHitResult Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	return true;
}

void ADFWeapon_Hitscan::ServerNotifyHit_Implementation(const FHitResult Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
...
bool ADFWeapon_Hitscan::ServerNotifyMiss_Validate(FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	return true;
}

void ADFWeapon_Hitscan::ServerNotifyMiss_Implementation(FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	//Trail FX
	const FVector Origin = GetMuzzleLocation();
...
void ADFWeapon_Hitscan::ProcessInstantHit(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir, int32 RandomSeed, float ReticleSpread)
{
...
void ADFWeapon_Hitscan::ProcessInstantHit_Confirmed(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir, int32 RandomSeed, float ReticleSpread)
{
...
bool ADFWeapon_Hitscan::ShouldDealDamage(AActor* TestActor) const
{
	if (TestActor)
	{
...
/** handle damage */
void ADFWeapon_Hitscan::DealDamage(const FHitResult& Impact, const FVector& ShootDir)
{
...
void ADFWeapon_Hitscan::FireWeapon() 
{
...
float ADFWeapon_Hitscan::GetWeaponSpread() const
{
...
void ADFWeapon_Hitscan::OnBurstFinished() 
{
	Super::OnBurstFinished();
}

/** called in network play to do the cosmetic fx  */
void ADFWeapon_Hitscan::OnRep_HitNotify()
{
	SimulateInstantHit(HitNotify.Origin, HitNotify.RandomSeed, HitNotify.ReticleSpread);
}

void ADFWeapon_Hitscan::SimulateInstantHit(const FVector& Origin, int32 RandomSeed, float ReticleSpread)
{
	FRandomStream WeaponRandomStream(RandomSeed);
...
void ADFWeapon_Hitscan::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME_CONDITION(ADFWeapon_Hitscan, HitNotify, COND_SkipOwner);
}
